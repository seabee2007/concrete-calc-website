...
4. writes `dist/index.html` for `/` and `dist/<route>.html` for every other route (flat files: Netlify's pretty URLs serve `/pricing` from `pricing.html` with a 200, whereas `pricing/index.html` would make `/pricing` a 301 to `/pricing/` and contradict the canonical), failing the build if a route has no title or H1, or references an asset the client build did not emit.
...
Verification: `npm run verify:seo` checks each prerendered file for the expected title, description, canonical and H1; `src/lib/prerender.test.ts` renders every route in Vitest. Local preview: `npm run preview`, then open `/pricing.html` (Vite's preview server does not apply pretty URLs; Netlify serves the same file at `/pricing`).
